        ...
        public List<Post> GetAllWithComments()
        {
            using (var conn = Connection)
            {
              ...
              ORDER BY p.DateCreated";
                    ...
                    //creating a readline for information-listen to whats about to happen
                    var reader = cmd.ExecuteReader();

                    //creating a variable and storing it as a list of post
                    var posts = new List<Post>();
                    //so now we can say while the reader is reading...
                    while (reader.Read())
                    {
                        var postId = DbUtils.GetInt(reader, "PostId");
                        //creating another variable that matches the first id it finds, it wont duplicate any of the post in the list its creating above on line 86 by p.Id == postId
                        var existingPost = posts.FirstOrDefault(p => p.Id == postId);
                        //if the post doesnt exist then it will create the post and add it to the list(thats what if(existingPost == null) means
                        ...
                        //if a comment exist, make sure it is in the list of the existingpost
